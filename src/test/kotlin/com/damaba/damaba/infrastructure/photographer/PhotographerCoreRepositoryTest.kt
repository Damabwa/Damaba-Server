...
class PhotographerCoreRepositoryTest @Autowired constructor(
    ...
    fun `(GET) 사진작가를 id로 단건 조회한다`() {
        ...
        val photographerJpaEntity = createPhotographerJpaEntity(id = userId)
        ...
        assertThat(result.address?.sido).isEqualTo(photographerJpaEntity.address?.sido)
        assertThat(result.address?.sigungu).isEqualTo(photographerJpaEntity.address?.sigungu)
        assertThat(result.address?.jibunAddress).isEqualTo(photographerJpaEntity.address?.jibunAddress)
        assertThat(result.address?.roadAddress).isEqualTo(photographerJpaEntity.address?.roadAddress)
    }
